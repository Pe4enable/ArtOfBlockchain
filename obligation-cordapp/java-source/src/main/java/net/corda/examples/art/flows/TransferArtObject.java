...
import static net.corda.examples.art.ArtObjectContract.ARTOBJECT_CONTRACT_ID;

public class TransferArtObject {
    @StartableByRPC
    @InitiatingFlow
    public static class Initiator extends ArtObjectBaseFlow {
        ...
        private final Step GET_ARTOBJECT = new Step("Obtaining artObject from vault.");
        ...
        private final ProgressTracker progressTracker = new ProgressTracker(GET_ARTOBJECT, CHECK_INITIATOR, BUILD_TRANSACTION, SIGN_TRANSACTION, SYNC_OUR_IDENTITY, COLLECT_SIGS, SYNC_OTHER_IDENTITIES, FINALISE);
        ...
        public SignedTransaction call() throws FlowException {
            // Stage 1. Retrieve artObject with the correct linear ID from the vault.
            progressTracker.setCurrentStep(GET_ARTOBJECT);
            final StateAndRef<ArtObject> artObjectToTransfer = getArtObjectByLinearId(linearId);
            final ArtObject inputArtObject = artObjectToTransfer.getState().getData();

            final Party borrower = getBorrowerIdentity(inputArtObject);
            ...
            if (!getOurIdentity().equals(getLenderIdentity(inputArtObject))) {
                throw new IllegalStateException("ArtObject transfer can only be initiated by the lender.");
            }

            // Stage 3. Create the new artObject state reflecting a new lender.
            ...
            final ArtObject transferredArtObject = createOutputArtObject(inputArtObject, newLenderSession);

            // Stage 4. Create the transfer command.
            final List<PublicKey> signerKeys = new ImmutableList.Builder<PublicKey>()
                    .addAll(inputArtObject.getParticipantKeys())
                    .add(transferredArtObject.getLender().getOwningKey()).build();
            final Command transferCommand = new Command<>(new ArtObjectContract.Commands.Transfer(), signerKeys);
            ...
            final TransactionBuilder builder = new TransactionBuilder(getFirstNotary())
                    .addInputState(artObjectToTransfer)
                    .addOutputState(transferredArtObject, ARTOBJECT_CONTRACT_ID)
                    .addCommand(transferCommand);
            builder.verify(getServiceHub());

            // Stage 6. Sign the transaction using the key we originally used.
            progressTracker.setCurrentStep(SIGN_TRANSACTION);
            final SignedTransaction ptx = getServiceHub().signInitialTransaction(builder, inputArtObject.getLender().getOwningKey());
            ...
            final SignedTransaction stx = subFlow(new CollectSignaturesFlow(
                    ptx,
                    sessions,
                    ImmutableList.of(inputArtObject.getLender().getOwningKey()),
        ...
        @Suspendable
        private AbstractParty getLenderIdentity(ArtObject inputArtObject) {
            if (inputArtObject.getLender() instanceof AnonymousParty) {
                return resolveIdentity(inputArtObject.getLender());
            } else {
                return inputArtObject.getLender();
            }
        }

        @Suspendable
        private ArtObject createOutputArtObject(ArtObject inputArtObject, FlowSession newLenderSession) throws FlowException {
            if (anonymous) {
                final LinkedHashMap<Party, AnonymousParty> anonymousIdentities = subFlow(new SwapIdentitiesFlow(newLenderSession));
                return inputArtObject.withNewLender(anonymousIdentities.get(newLenderSession.getCounterparty()));
            } else {
                return inputArtObject.withNewLender(newLender);
            }
        }

        @Suspendable
        private Party getBorrowerIdentity(ArtObject inputArtObject) {
            if (inputArtObject.getBorrower() instanceof AnonymousParty) {
                return resolveIdentity(inputArtObject.getBorrower());
            } else {
                return (Party) inputArtObject.getBorrower();
            }
        }
    }
