...
import net.corda.examples.art.flows.IssueArtObject;
import net.corda.examples.art.flows.SettleArtObject;
import net.corda.examples.art.flows.TransferArtObject;
...
@Path("artObject")
public class ArtObjectApi {
    private final CordaRPCOps rpcOps;
    private final Party myIdentity;

    public ArtObjectApi(CordaRPCOps rpcOps) {
        this.rpcOps = rpcOps;
    ...
    public Map<Currency, Long> owedPerCurrency() {
        return rpcOps.vaultQuery(ArtObject.class).getStates()
                .stream()
    ...
    public List<ArtObject> artObjects() {
        List<StateAndRef<ArtObject>> statesAndRefs = rpcOps.vaultQuery(ArtObject.class).getStates();
    ...
    public Response issueArtObject(
        ...
        // 3. Start the IssueArtObject flow. We block and wait for the flow to return.
        try {
    ...
    @GET
    @Path("transfer-artObject")
    public Response transferArtObject(
        ...
        try {
            final FlowHandle flowHandle = rpcOps.startFlowDynamic(
                    TransferArtObject.Initiator.class,
                    linearId, newLender, true);

            flowHandle.getReturnValue().get();
            final String msg = String.format("ArtObject %s transferred to %s.", id, party);
            return Response.status(CREATED).entity(msg).build();
        } catch (Exception e) {
            return Response.status(BAD_REQUEST).entity(e.getMessage()).build();
        }
    ...
    @GET
    @Path("settle-artObject")
    public Response settleArtObject(
            @QueryParam(value = "id") String id,
            @QueryParam(value = "amount") int amount,
            @QueryParam(value = "currency") String currency) {
        ...
        try {
            final FlowHandle flowHandle = rpcOps.startFlowDynamic(
                    SettleArtObject.Initiator.class,
                    linearId, settleAmount, true);

            flowHandle.getReturnValue().get();
            final String msg = String.format("%s %s paid off on ArtObject id %s.", amount, currency, id);
            return Response.status(CREATED).entity(msg).build();
        } catch (Exception e) {
            return Response.status(BAD_REQUEST).entity(e.getMessage()).build();
        }
